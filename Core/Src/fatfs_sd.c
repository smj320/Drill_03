...
/**
 * Sortware SPI
 * 8MHzクロックで、CLKは7usecくらい(142kHz)
 * 512byte送信には
 * @param txData
 * @return
 */
#if 0
uint8_t xmit_sspi(uint8_t txData) {
    uint8_t rxData = 0;
    for (int i = 7; 0 <= i; i--) {
        GPIO_PinState txBit = (txData >> i) & 0x01 ? GPIO_PIN_SET : GPIO_PIN_RESET;
        HAL_GPIO_WritePin(SSPI_MOSI_GPIO_Port, SSPI_MOSI_Pin, txBit);
        HAL_GPIO_WritePin(SSPI_CLK_GPIO_Port, SSPI_CLK_Pin, GPIO_PIN_SET);
        uint8_t rxBit = HAL_GPIO_ReadPin(SSPI_MISO_GPIO_Port, SSPI_MISO_Pin);
        HAL_GPIO_WritePin(SSPI_CLK_GPIO_Port, SSPI_CLK_Pin, GPIO_PIN_RESET);
        rxData = rxData | ((rxBit & 0x01) << i);
    }
    return rxData;
}
#endif
...
static void SPI_TxByte(BYTE data) {
    static uint8_t txData[10], rxData[10];
    txData[0] = data;
