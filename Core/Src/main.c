...
int8_t BME280_i2c_read(uint8_t id, uint8_t reg_addr, uint8_t *data, uint16_t len) {
    ...
    s = HAL_I2C_Mem_Read(&hi2c1, id << 1, reg_addr, I2C_MEMADD_SIZE_8BIT, data, len, 100);
...
int main(void)
{
  ...
  /* OpenOCD の reset-init が PLL(64MHz) を有効にしたまま main に入るため、
     SYSCLK=PLL の状態だと HAL_RCC_OscConfig() が PLL を再設定できず HAL_ERROR を返す。
     先に RCC をリセット状態(HSI)に戻す。 */
  HAL_RCC_DeInit();
  /* USER CODE END Init */
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin : SD_BSY_Pin */
  GPIO_InitStruct.Pin = SD_BSY_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  HAL_GPIO_Init(SD_BSY_GPIO_Port, &GPIO_InitStruct);

  /*Configure GPIO pin : BUTYL_Pin */
  GPIO_InitStruct.Pin = BUTYL_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
  GPIO_InitStruct.Pull = GPIO_PULLUP;
  HAL_GPIO_Init(BUTYL_GPIO_Port, &GPIO_InitStruct);
